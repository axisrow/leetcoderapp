...
const AIPuzzle = () => {
  ...
  const run = useCallback(async () => {
    ...
    const prompt = `Write 1 quiz without answer for improving Javascript skills:
Topic: ${randomTheme}
...
Number of questions: ${randomNum}`;
    ...
    try {
      await generateWithFallback(prompt, {
        ...
        logPrefix: '[Puzzles:Quiz]',
        onChunk: (fullText) => setReslt(fullText),
      });
      setIsLoading(false);
    } catch (error) {
      console.error('[Puzzles] ✗ Error (Quiz):', error.message);
      setReslt('⚠️ Не удалось сгенерировать квиз. Попробуйте позже.');
      setIsLoading(false);
    }
  }, []);
  ...
  const submitAnswer = useCallback(async () => {
    ...
    const prompt = `Analyze the user's answer to this quiz and provide constructive feedback. Tell them if they're on the right track and explain why their answer is correct or incorrect. Be encouraging and educational.
...
Provide feedback without revealing the correct answer yet. Focus on their thinking process.`;
    ...
    try {
      await generateWithFallback(prompt, {
        temperature: 0.9,
        maxOutputTokens: 2048,
        logPrefix: '[Puzzles:Feedback]',
        onChunk: (fullText) => setFeedback(fullText),
      });
      setIsLoading(false);
    } catch (error) {
      console.error('[Puzzles] ✗ Error (Feedback):', error.message);
  ...
  const runCheck = useCallback(async () => {
    ...
    const prompt = `What is the right answer to this quiz, and why?\n\nQuiz:\n${reslt}`;

    try {
      await generateWithFallback(prompt, {
        temperature: 0.9,
        maxOutputTokens: 2048,
        logPrefix: '[Puzzles:Answer]',
        onChunk: (fullText) => setAnswers(fullText),
      });
      setShowCorrectAnswer(true);
      setIsLoading(false);
    } catch (error) {
      console.error('[Puzzles] ✗ Error (Answer):', error.message);
      setAnswers('⚠️ Не удалось получить ответ. Попробуйте позже.');
      setShowCorrectAnswer(true);
      setIsLoading(false);
    }
  }, [reslt]);
